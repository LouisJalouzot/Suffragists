...
def plot_layout(layout: pd.DataFrame, save_path: Path):
    plt.figure()
    if "page" in layout:
        ...
        palette = "inferno"
    ax = sns.scatterplot(
        data=layout, x="x_center", y=-layout.y_center, hue=hue, palette=palette
    )
...
def compute_perplexity(text, model, tokenizer, batch_size=32):
    ...
    return torch.exp(torch.stack(nlls).median()).item()
...
def ocr_and_cluster(
    ...
    model_name="distilbert/distilgpt2",  # "meta-llama/Llama-3.2-1B",
):
    ...
    with joblib_progress(description="OCRing", total=n_images):
        for issue, layout in Parallel(n_jobs=-2)(
            delayed(
                ...
                issue,
                image_path,
                output_path / issue / "ocr" / f"{i + 1}.csv",
            )
            ...
            try:
                ...
                for scan, layout in enumerate(layouts, 1):
                    ...
                    if clustering:
                        ...
                        # Find pages
                        image_width = (
                            layout.x_center.max() - layout.x_center.min()
                        )
                        image_height = (
                            layout.y_center.max() - layout.y_center.min()
                        )
                        ...
                        for page in layout.page.unique():
                            ...
                            for n_cols in [2, 3, 4]:
                                kmeans = KMeans(
                                    n_clusters=n_cols, random_state=0
                                )
                            ...
                            layout.loc[slice, "column"] = res[best_n_cols][
                                "cols"
                            ]
